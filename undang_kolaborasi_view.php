...
<html lang="en">
<head>
	<title>Undangan Kolaborasi</title>
	<meta charset="UTF-8">
...
<body>
	...
	<section class="sec-relate-product bg0 p-t-45 p-b-105">
		<div class="container">
			<a href='modul_view.php?md=<?php echo $uuidporto;?>&fk=&uv='><button class="btn btn-primary ms-auto"><i class="fa-solid fa-arrow-left"></i> Kembali</button></a>								
            <form action='undang_kolaborasi_view_hapus.php' style="float:right" method="POST" 
			onsubmit="return confirm('Yakin ingin menghapus undangan kolaborasi ini?');">
                <input type='hidden' name='uuidporto' value='<?php echo $uuidporto;?>'/>
                <input type='hidden' name='id_undangan' value='<?php echo $id_undangan;?>'/>
				<button class="btn btn-danger ms-auto"><i class="fa-solid fa-trash"></i> Batalkan/Hapus</button>							
			</form>
			
			<h3 class="ltext-106 cl5 txt-center mt-5">
				Undangan Kolaborasi
			</h3>
            <form class="w-full" method="post" action="undang_kolaborasi_view_proses.php" enctype="multipart/form-data">
                <div class="flex-w flex-m p-t-50 p-b-23">
                </div>
                <input id="name" type="text" name="uuidporto" value="<?php echo $uuidporto;?>" hidden/>
                <?php
                    $sql = "SELECT ug.id_jenis_kerjasama, ug.keterangan keterangan_undangan, ug.st_konfirmasi, p.uuiduser, id_kat_porto, nama_porto, filename, p.keterangan, thumbnail_filename,
                    substring(u.fname FROM '\S+$') fname, angkatan, u.kode_univ, u.kode_fakultas, nama_prodi, nama_univ, nama_fakultas, st_konfirmasi, kontak_yg_diberikan, u.no_hp, u.email
                    from d_portofolio p
                    ...
                    left join f_undangan_kerjasama ug on ug.uuiduserdiundang = p.uuiduser
                    where (p.uuidporto = '$uuidporto')
                    and (id_undangan = $id_undangan)";
                    $hasil = $db->query($sql);
                    $baris = $hasil->fetch(PDO::FETCH_ASSOC);

					if ($baris['st_konfirmasi'] == 'Y') {
						$st_konfirmasi = 'Disetujui';
					}
					else if ($baris['st_konfirmasi'] == 'N') {
						$st_konfirmasi = 'Ditolak';
						$kontak = '';
					}


					if ($baris['kontak_yg_diberikan'] == 'HP' AND $baris['st_konfirmasi'] == 'Y') {
						$kontak = $baris['no_hp'];
					}
					else if ($baris['kontak_yg_diberikan'] == 'EMAIL' AND $baris['st_konfirmasi'] == 'Y') {
						$kontak = $baris['email'];
					}
					else {
						$kontak = '';
				...
				<?php
					if ($baris['st_konfirmasi'] !== NULL) {
				?>
				<div class="w-full py-1 px-3 shadow-md" style="background:#ffce6d; margin-top:-3%">
					<h2 class="text-xl font-semibold mb-4"></h2><br>
					<div class="row p-b-25" style="margin-top:-2%">
						<div class="col-sm-6 p-b-5">
							<label class="stext-102 cl3" for="review">Status Persetujuan</label>
							<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
								<input value="<?php echo $st_konfirmasi;?>" class="size-111 bor8 stext-102 cl2 p-lr-20" id="st_konfirmasi" type="text" name="st_konfirmasi" disabled>
							</div>
						</div>
						<div class="col-sm-6 p-b-5">
							<label class="stext-102 cl3" for="review">Kontak Lanjutan yang Diberikan</label>
							<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
								<input value="<?php echo $kontak;?>" class="size-111 bor8 stext-102 cl2 p-lr-20" id="kontak_yg_diberikan" type="text" name="kontak_yg_diberikan" disabled>
							</div>
						</div>
					</div>	
				</div>
                    ...
                    <div class="col-sm-12 p-b-5">
						...
						<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
							<select class="form-control" name="id_jenis_kerjasama" required autofocus>
                                ...
                                ?>
                                    <option value='<?php echo $id_jenis_kerjasama;?>' <?php echo ($barisb['id_jenis_kerjasama'] == $baris['id_jenis_kerjasama']) ? 'selected' : ''; ?>><?php echo $nama_jenis_kerjasama;?></option>
				...
				<div class="row p-b-25" style="margin-top:-2%">
					<div class="col-12 p-b-5">
						...
						<textarea required maxlength="300" class="size-110 bor8 stext-102 cl2 p-lr-20 p-tb-10" style="height:150px" placeholder="Deskripsikan alasan kolaborasi secara jelas" id="deskripsi" name="keterangan"><?php echo $baris['keterangan_undangan'];?></textarea>
					</div>
				</div>
                ...
                <button 
                    ...
                    type="submit" 
                    name="update"
                    onclick="return confirm('Yakin ingin memperbarui undangan kolaborasi ini ?')">
