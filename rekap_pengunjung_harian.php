...
<html lang="en">
<head>
	<title>Rekap Pengunjung Harian</title>
	<style>
	#rekap_pengunjung_harian_mobile {background:#4052cb}
	#nav_rekap_harian {color:#717fe0}
...
<body>
	...
	<div class="container">
		<div class="bread-crumb flex-w p-l-25 p-r-15 p-t-30 p-lr-0-lg">
			<a href="rekap_pengunjung_harian.php" class="stext-109 cl8 hov-cl1 trans-04">
				Rekap Pengunjung Harian
	...
	<section class="bg0 p-t-23 p-b-140">
		<div class="container">
			...
			<div class="flex-w flex-sb-m p-b-52">
				...
				$uuidportofilter  = "";
                $bulan_filter = date('m');
                //$bulan_filter = ltrim($bulan_filter, '0');
				...
				<div class="dis-none panel-search w-full p-t-10 p-b-15">
					...
					<div class="bor8 dis-flex p-l-15">
							<input class="mtext-107 cl2 size-114 plh2 p-r-15" type="text" name="nama_kos" placeholder="Search">
				...
				<div class="dis-none panel-filter w-full p-t-10">
					<form method="POST" action="#" style="width: 100%; margin: 0 auto; background-color: #f0f0f0; padding: 20px; border-radius: 8px;">
						<div style="display: flex; flex-wrap: wrap; justify-content: space-between; margin-bottom: 20px;">
							...
							<div style="flex: 1; margin-right: 10px; margin-bottom: 10px;">
								...
								<select name="uuidportofilter" id="jenis-sewa" style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;">
                                    ...
                                    <?php
										...
										<option value=<?php echo $baris2['uuidporto'];?> <?= ($baris2['uuidporto'] == $uuidportofilter) ? 'selected' : '' ?>><?php echo $nama_porto;?></option>
									<?php
										}
									?>
								</select>
                            ...
                            <!-- Bulan -->
							<div style="flex: 1; margin-right: 10px; margin-bottom: 10px;">
								<label for="jenis-sewa" style="font-size: 16px; color: #333; display: block; margin-bottom: 5px;">Bulan :</label>
								<select name="bulan_filter" id="jenis-sewa" style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;">
						...
						<!-- Tombol Pencarian -->
						<button name="refresh" type="submit" style="width: 100%; padding: 10px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">
            ...
            <?php
                $jumlah_hari = cal_days_in_month(CAL_GREGORIAN, $bulan_filter, 2026);
                for ($i=1; $i<=$jumlah_hari; $i++) {
                    $tgl_jadi = '2026-'.$bulan_filter.'-'.$i;
                    //echo $tgl_jadi;
                    
                    $sql5 = "SELECT sum(inc_klik) hsl FROM f_klik_pengunjung_log log
                    LEFT JOIN d_portofolio rk ON log.uuidporto = rk.uuidporto
                    WHERE ('$uuidportofilter' = '' or log.uuidporto = '$uuidportofilter') 
                    AND log.createdate::date = '$tgl_jadi' AND rk.uuiduser = '$uuiduser'";
                    $hasil5 = $db->query($sql5);
                    $baris5 = $hasil5->fetch(PDO::FETCH_ASSOC);
                    $hasil_hitung = $baris5['hsl'] ?? 0;
                    //echo $hasil_hitung. "<br>";
                    $arr_hasil[] = $hasil_hitung;
                    $arr_tgl[] = $i;
                }
                //var_dump ($arr_hasil);
                ...
                $nama_bulan = $bulanIndo[$bulan_filter]; // April

                if (!isset($baris3['nama_porto'])) {
                    $kal = 'Statistik Pengunjung Semua Portofolio Bulan '. $nama_bulan;
                }
                else {
                    $kal = 'Statistik Pengunjung Portofolio '. $baris3['nama_porto']. ' bulan '. $nama_bulan;
                }
            ?>
            <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-3xl">
                ...
                <script>
                    const dataKlik = <?php echo json_encode($arr_hasil); ?>;
                    const labels = <?php echo json_encode($arr_tgl); ?>;
