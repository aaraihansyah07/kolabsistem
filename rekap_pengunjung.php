...
<html lang="en">
<head>
	<title>Rekap Pengunjung Bulanan</title>
	<style>
	#rekap_pengunjung_mobile {background:#4052cb}
	#nav_rekap_bulanan  {color:#717fe0}
...
<body>
	...
	<div class="container">
		<div class="bread-crumb flex-w p-l-25 p-r-15 p-t-30 p-lr-0-lg">
			<a href="rekap_pengunjung.php" class="stext-109 cl8 hov-cl1 trans-04">
				Rekap Pengunjung Bulanan
		...
		<div class="container">
			...
			<div class="flex-w flex-sb-m p-b-52">
				...
				if (isset($_POST['refresh'])) {;
					$uuidportofilter = $_POST['uuidportofilter'];
				}
				?>
				<!-- Berdasarkan Nama -->
				<div class="dis-none panel-search w-full p-t-10 p-b-15">
					...
					<div class="bor8 dis-flex p-l-15">
							<input class="mtext-107 cl2 size-114 plh2 p-r-15" type="text" name="nama_porto" placeholder="Search">
					...
					<form method="POST" action="#" style="width: 100%; margin: 0 auto; background-color: #f0f0f0; padding: 20px; border-radius: 8px;">
						...
						<div style="display: flex; flex-wrap: wrap; justify-content: space-between; margin-bottom: 7px;">
							...
							<div style="flex: 1; margin-right: 10px; margin-bottom: 10px;">
								<label for="jenis-sewa" style="font-size: 16px; color: #333; display: block; margin-bottom: 5px;">Portofolio</label>
								<select name="uuidportofilter" id="jenis-sewa" style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;">
									<option value="" <?= ($uuidportofilter == '') ? 'selected' : '' ?>>All</option>  
									<?php
										$sql2 = "SELECT uuidporto, nama_porto nama_porto
										FROM d_portofolio m
										left join d_kategori_portofolio p on p.id_kat_porto = m.id_kat_porto
										where uuiduser = '$uuiduser'";
										$hasil2 = $db->query($sql2);
										while ($baris2 = $hasil2->fetch(PDO::FETCH_ASSOC)) {
											$uuidporto = $baris2['uuidporto'];
											$nama_porto = $baris2['nama_porto'];
									?>	
										<option value=<?php echo $uuidporto;?> <?= ($uuidporto == $uuidportofilter) ? 'selected' : '' ?>><?php echo $nama_porto;?></option>
									<?php
										}
									?>
								</select>
						...
						<!-- Tombol Pencarian -->
						<button name="refresh" type="submit" style="width: 100%; padding: 10px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;"><i class="fa-solid fa-refresh"></i> Refresh</button>
					</form>
				</div>
			</div>
            
            <?php
                $sql3 = "SELECT nama_porto FROM d_portofolio WHERE uuidporto = '$uuidportofilter'";
                $hasil3 = $db->query($sql3);
                $baris3 = $hasil3->fetch(PDO::FETCH_ASSOC);

                if (!isset($baris3['nama_porto'])) {
                    $kal = 'Statistik Pengunjung Semua Portofolio';
                }
                else {
                    $kal = 'Statistik Pengunjung Portofolio '. $baris3['nama_porto'];
                }

                $sql4 = "SELECT rk.uuiduser, 
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 1 THEN inc_klik ELSE 0 END) jan, 
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 2 THEN inc_klik ELSE 0 END) feb,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 3 THEN inc_klik ELSE 0 END) mar,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 4 THEN inc_klik ELSE 0 END) apr,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 5 THEN inc_klik ELSE 0 END) mei,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 6 THEN inc_klik ELSE 0 END) jun,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 7 THEN inc_klik ELSE 0 END) jul,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 8 THEN inc_klik ELSE 0 END) agu,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 9 THEN inc_klik ELSE 0 END) sep,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 10 THEN inc_klik ELSE 0 END) okt,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 11 THEN inc_klik ELSE 0 END) nov,
                SUM(CASE WHEN EXTRACT(MONTH FROM logs.createdate) = 12 THEN inc_klik ELSE 0 END) dese
                FROM f_klik_pengunjung_log logs
                LEFT JOIN d_portofolio rk on rk.uuidporto = logs.uuidporto
                WHERE EXTRACT(YEAR FROM logs.createdate) = 2026 
                AND ('$uuidportofilter' = '' or logs.uuidporto = '$uuidportofilter')
                AND rk.uuiduser = '$uuiduser' GROUP BY rk.uuiduser";
                ...
                $dataKlik = [$jan, $feb, $mar, $apr, $mei, $jun, $jul, $agu, $sep, $okt, $nov, $dese];
            ?>
            <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-3xl">
                    <h2 class="text-xl font-bold text-center mb-4"><?php echo $kal;?></h2>
                    <canvas id="klikChart" height="120"></canvas>
                </div>
                <script>
                    const dataKlik = <?php echo json_encode($dataKlik); ?>;
                    const labels = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];

                    const ctx = document.getElementById('klikChart').getContext('2d');
                    new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: labels,
