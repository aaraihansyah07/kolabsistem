	...
	<section class="bg0 p-t-23 p-b-140">
		<div class="container">
            ...
            <div class="space-y-4">
                <!-- Notifikasi 1 -->
                 <?php
                    $sql = "select h.uuidinvoice, h.no_invoice, h.nama_pemesan, to_char(h.tgl_pesanan, 'DD MONTH YYYY HH24:MI') tgl_pesanan, h.alamat, h.no_hp_pemesan,
                    d.netto, d.uuidproduk
                    from f_invoice_shoficraft_hdr h 
                    left join f_invoice_shoficraft_dtl d on d.uuidinvoice = h.uuidinvoice
                    where h.username_pemesan = '$user' order by tgl_pesanan desc";
                    $hasil = $db->query($sql);
                    while ($baris = $hasil->fetch(PDO::FETCH_ASSOC)) {
                        $uuidproduk = $baris['uuidproduk'];
                        $sql2 = "SELECT nama_produk from produk_shoficraft where uuidproduk = '$uuidproduk'";
                ...
                <div class="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm">
					<div class="flex-shrink-0">
						<div class="w-10 h-10 bg-yellow-100 text-yellow-600 rounded-full flex items-center justify-center">
							<i class="zmdi zmdi-edit text-xl"></i>
						</div>
					</div>
                    <div class="ml-4">
                        <p class="text-gray-700 text-sm sm:text-base">
                            <span class="font-semibold text-red-600"><?php echo $baris['no_invoice'];?></span>
                        </p>
						<p class="text-sm text-black-400 mt-1"><a style='color:#4094c6' href='katalog_partner_detail.php?upr=<?php echo $baris['uuidproduk'];?>&iv=N'><?php echo $baris2['nama_produk'];?></a></p>
                        <p class="text-sm text-black-400 mt-1">Total : Rp. <?php echo number_format($baris['netto']);?></p>
						<p class="text-xs text-gray-400 mt-1"><?php echo $baris['tgl_pesanan'];?></p><br>
                        <div>
                            <form method="POST" action="invoice_download.php">
                                <input type="hidden" name="no_invoice" value="<?php echo $baris['no_invoice'];?>"/>
                                <button name="view_invoice" class="btn btn-success w-100 d-flex align-items-center justify-content-center gap-1 py-1">
                                    Download Invoice
                                </button>
                            </form>
                        </div>
