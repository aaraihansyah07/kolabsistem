...
<body>
	...
	<div class="container">
		<br>
		<div id="cek_kolab" style="background:#f5ba46; width:97%; margin:0 auto; padding:3px; text-align:center; border-radius:10px">
			<?php
				$sqla = "SELECT uuiduserdiundang, st_konfirmasi, id_undangan FROM f_undangan_kerjasama
				WHERE uuiduserpengundang = '$uuiduser' AND uuiduserdiundang = (SELECT uuiduser from d_portofolio where uuidporto = '$uuidporto')
				order by st_konfirmasi desc nulls first
				LIMIT 1";
				$hasila = $db->query($sqla);
				$barisa = $hasila->fetch(PDO::FETCH_ASSOC);

				if (!isset($barisa['uuiduserdiundang'])) {
					echo "<style>#cek_kolab {display:none}</style>";
				}
				else {
					$sqlc = "SELECT uuiduserdiundang, st_konfirmasi, id_undangan FROM f_undangan_kerjasama
					WHERE uuiduserpengundang = '$uuiduser' AND uuiduserdiundang = (SELECT uuiduser from d_portofolio where uuidporto = '$uuidporto')
					AND st_konfirmasi is null LIMIT 1";
					$hasilc = $db->query($sqlc);
					$barisc = $hasilc->fetch(PDO::FETCH_ASSOC);

					if (!isset($barisc['uuiduserdiundang'])) {
						echo "<style>#cek_kolab {display:block}</style>";
						echo "<p style='font-weight:bold; color:black; font-size:14'>Anda sudah pernah berhasil berkolaborasi dengan user ini, Keren!!. Anda juga bisa mengundang user ini untuk berkolaborasi lagi</p>";
					}
					else {
						echo "<p style='font-weight:bold; color:black; font-size:14'>Anda sudah pernah mengundang user ini untuk kolaborasi, tunggu konfirmasi persetujuan dari dia ya...</p>";
						
						echo "<form action='undang_kolaborasi_view.php' method='POST' style='margin-right: 3'>";
							echo "<input type='hidden' name='md' value='".$uuidporto."'/>";
							echo "<input type='hidden' name='id_undangan' value='".$barisc['id_undangan']."'/>";
							echo "<button name='view' style='font-weight:bold; text-decoration:underline; background-color:none; color: #4875e5; border: none; padding: 5px 10px; border-radius: 10px; cursor: pointer; font-size: 14px;'>Lihat Undangan Kolaborasi</button>";								
						echo "</form>";
					}
				}
			?>
		...
		<div class="bread-crumb flex-w p-l-25 p-r-15 p-t-30 p-lr-0-lg">
			<a href='index.php?md=<?php echo $uuidporto;?>&fk=<?php echo $id_kat_porto;?>&uv=<?php echo $kode_univ;?>'><button id="btnUpdate" name="edit"  type="submit" class="btn btn-primary ms-auto">
				<i class="fa-solid fa-arrow-left"></i> Ke Home
			</button><a/a>
			<!-- <a href='index.php?md=<?php echo $uuidporto;?>&fk=<?php echo $id_kat_porto;?>&uv=<?php echo $kode_univ;?>'><button style="top: 10px; right: 10px; background-color: #717fe0; color: white;
		...
		<div class="bread-crumb flex-w p-l-25 p-r-15 p-t-20 p-lr-0-lg">
			<a href="index.php?md=<?php echo $uuidporto;?>&fk=<?php echo $id_kat_porto;?>&uv=<?php echo $kode_univ;?>" class="stext-109 cl8 hov-cl1 trans-04">
				Home
	...
	<div style="display: flex; justify-content: center; padding: 0.4rem;">
		<form class="radio-group" method="GET" action="ruangan_kosan_view.php">
			<input type="hidden" name="rk" value="<?php echo htmlspecialchars($uuidporto); ?>">
		</form>
	</div>
		...
		<div class="container">
			<div class="row">
				<div class="col-md-6 col-lg-7 p-b-30">
					<div class="p-l-25 p-r-30 p-lr-0-lg">
						<?php
							if (isset($_POST["klik_$uuidporto"]) AND isset($uuiduser)) {
								$sql9 = "SELECT count(1) jml_visitor FROM f_klik_pengunjung_log
								WHERE createdate::date =CURRENT_DATE AND userklik = '$uuiduser' AND uuidporto = '$uuidporto'";
								$hasil9 = $db->query($sql9);
								$baris9 = $hasil9->fetch(PDO::FETCH_ASSOC);

								if ($baris9['jml_visitor'] == 0) {
									$sql3 = "INSERT INTO f_klik_pengunjung_log(uuidporto, ip_address, userklik, inc_klik) VALUES (:uuidporto, :ip_address, :uuiduser, 1)";
									$stmt3 = $db->prepare($sql3);
									$stmt3->execute(['uuidporto' => $uuidporto, 'ip_address' => $ip_address, 'uuiduser' => $uuiduser]);

									$sql4 = "SELECT sum(inc_klik) jml_klik from f_klik_pengunjung_log where uuidporto = '$uuidporto'";
									$hasil4 = $db->query($sql4);
									$baris4 = $hasil4->fetch(PDO::FETCH_ASSOC);

									$jml_klik = $baris4['jml_klik'];
									
									$sql5 = "UPDATE d_portofolio set jml_klik = $jml_klik where uuidporto = :uuidporto";
									$stmt5 = $db->prepare($sql5);
									$stmt5->execute(['uuidporto' => $uuidporto]);
								}
							}

							$sql = "SELECT u.angkatan, m.nama_porto, m.keterangan, m.uuidporto, substring(u.fname FROM '\S+$') fname, m.thumbnail_filename, m.filename,
							m.jml_klik, j.nama_kategori_porto, v.nama_univ, f.nama_fakultas, p.nama_prodi, u.kode_fakultas, f.nama_fakultas, u.gender, u.kode_univ
							from d_portofolio m 
				...
				<div class="col-md-6 col-lg-5 p-b-30">
					<?php
						$sql2 = "SELECT uuiduser FROM d_portofolio WHERE uuidporto = '$uuidporto'";
					...
					<div class="size-204 flex-w flex-m respon6-next">
						<form action='modul_edit.php?rk=<?php echo $uuidporto;?>&st=N' method='POST' style="margin-right: 3%;">
							<input type='hidden' name='to_detail' value='<?php echo $uuidporto;?>'/>
							<button id="btnUpdate" name="edit"  type="submit" class="btn btn-success ms-auto">
						...
						<form action='modul_hapus.php?md=<?php echo $uuidporto;?>' 
							method='POST' 
							style="margin-right: 3%;"
							onsubmit="return confirm('Apakah Anda yakin ingin menghapus portofolio ini? Mahasiswa/dosen/pencari talent tidak akan lagi bisa melihat portofolio ini');">
							<button name="hapus" type="submit" class="btn btn-danger ms-auto">
					...
					<?php
						...
						if ($user !== "Login" AND $baris2['uuiduser'] !== $uuiduser) {
							$sql5 = "SELECT count(1) uuidporto from f_favourite_pencari_porto where uuiduserfav = '$uuiduser' and uuidporto = '$uuidporto' and jenis_notif = 'FAV'";
							$hasil5 = $db->query($sql5);
							$baris5 = $hasil5->fetch(PDO::FETCH_ASSOC);

							if ($baris5['uuidporto'] > 0) {
								echo "<button class='wishlist-btn active' data-ruangan='".$uuidporto."'>";
								echo "<i class='fa-solid fa-heart' style='color:red; font-size:160%'></i>";
								echo "</button>";
							}
							else {
								echo "<button class='wishlist-btn inactive' data-ruangan='".$uuidporto."'>";
								echo "<i class='fa-solid fa-heart' style='color:lightgray; font-size:160%'></i>";
								echo "</button>";								
							}
						}

					if ((!isset($barisa['uuiduserdiundang'])) or (isset($barisa['uuiduserdiundang']) AND $barisa['st_konfirmasi'] == 'Y')) {
						if ($baris2['uuiduser'] !== $uuiduser) {
					?>
						<br><br><form action='undang_kolaborasi.php' 
							method='POST' 
							style="margin-right: 3%;">
							<input type='hidden' name='uuidporto' value='<?php echo $uuidporto;?>'/>
							<button name="undang" class="btn btn-success ms-auto">
							<i class="fa-solid fa-people-arrows"></i> Undang Kolaborasi
							</button>
						</form>
					<?php
						}
					...
					<div class="p-r-50 p-t-5 p-lr-0-lg">
						...
						<p class="stext-102 cl3 whitespace-pre-line" style="margin-top:-6%">
						  <?= htmlspecialchars($baris['keterangan']) ?>
						</p>
					</div>
				</div>
				<div class="w-full bg-white p-6 rounded-xl shadow-md" style="margin-top:-2%">
					<h2 class="text-xl font-semibold mb-4">Portofolio :</h2>
					<form action='download_porto.php' method='POST' style="margin-right: 3%;">
						<input type='hidden' name='filename' value='<?php echo $baris['filename'];?>'/>	
						<button name="download_porto" class="btn btn-primary ms-auto">
							<i class="fa-solid fa-download"></i> Download
						</button>						
						<!-- <button name="download_porto" style="top: 10px; right: 10px; background-color: #717fe0; color: white; border: none; padding: 5px 10px; border-radius: 10px; cursor: pointer; font-size: 14px;">
							Download
						</button> -->
					</form>
			...
			<div class="p-b-7" style='margin-top:-7px'>
				<h3 class="ltext-103 cl5" style="font-size:20px">
					<?php
						$sql12 = "SELECT*FROM d_portofolio where uuidporto = '$uuidporto'";
			...
			<h3 class="ltext-103 cl5" style="font-size:20px; margin-top:-3%">
				<?php
					...
					$sql9 = "SELECT m.nama_porto, m.keterangan, m.uuidporto, substring(u.fname FROM '^\S+') fname, m.thumbnail_filename, m.filename,
					m.jml_klik, j.nama_kategori_porto, v.nama_univ, f.nama_fakultas, p.nama_prodi, u.kode_fakultas, u.kode_univ, u.gender
					from d_portofolio m 
					left join user_pengguna u on u.uuiduser = m.uuiduser
					left join d_univ v on v.kode_univ = u.kode_univ
					left join d_fakultas f on f.kode_fakultas = u.kode_fakultas
					left join d_prodi p on p.kode_prodi = u.kode_prodi
					left join d_kategori_portofolio j on j.id_kat_porto = m.id_kat_porto
					where u.st_active = 'Y' AND m.uuiduser = '$uuiduser_pemilik' AND m.uuidporto <> '$uuidporto'";
					$hasil9 = $db->query($sql9);

					$hasil9b = $db->query($sql9);
					$baris9b = $hasil9b->fetch(PDO::FETCH_ASSOC);
					if (!isset($baris9b['uuidporto'])) {
						$judul = '';
					}
					else {
						$judul = 'Portofolio Lain';
					}
					echo $judul;
				?>
					...
					<div class="flex overflow-x-auto no-scrollbar gap-4" id="track">
						<?php
							while ($baris9 = $hasil9->fetch(PDO::FETCH_ASSOC)) {
								$uuidportos_other = $baris9['uuidporto'];
						?>
						<div class="card">
							<a href="modul_view.php?md=<?php echo $baris9['uuidporto'];?>&fk=&uv=">
							<img src="uploads/thumbnail/<?php echo $baris9['thumbnail_filename'];?>" alt="IMG-PRODUCT" style="height:220px">
							</a><br>
							
							
							<a href="modul_view.php?md=<?php echo $baris9['uuidporto'];?>&fk=&uv=" class="stext-104 cl4 hov-cl1 trans-04 js-name-b2 p-b-6">
								<?php echo $baris9['nama_porto']; ?>
							</a>
