...
<html lang="en">
<head>
	<title>Promosi Mahasiswa</title>
	<style>
	#promosi_mahasiswa_mobile {background:#4052cb}
		...
		<div class="container">
			<div class="p-b-10">
				<h3 class="ltext-103 cl5">
					Usaha-usaha Keren Mahasiswa
				</h3>
				<p style="color:#525255; font-size:small">Upload usaha keren mahasiswa secara gratis untuk ditampilkan di highlight halaman depan</p><br>
				<?php
					if ($role_id == 1) {
				?>
                <a href='promosi_mahasiswa_upload.php'><button name="download_porto" class="btn btn-success ms-auto">
                    <i class="fa-solid fa-add"></i> Promosi Usaha Saya
                </button></a>
				<?php
					}
				?>
			...
			<div class="flex-w flex-sb-m p-b-52">
				<div class="flex-w flex-c-m m-tb-10">
					...
					<div class="flex-c-m stext-106 cl6 size-105 bor4 pointer hov-btn3 trans-04 m-tb-4 js-show-search">
						...
						Cari usaha mahasiswa berdasarkan kata kunci
					</div>
				</div>
				...
				if (!isset($_GET['fk'])) {
					$jenis_promosi_filter = '';
				}
				else {
					$jenis_promosi_filter = $_GET['fk'];
				}
				...
				if (isset($_POST['cari_promosi'])) {
					$nama_promosi_cari = "";
					$jenis_promosi_filter  = "";
					$nama_promosi_cari = $_POST['nama_promosi'];
				}

				if (isset($_POST['cari_promosi_filter'])) {
					//$harga_dari = $_POST['harga_dari'];
					//$harga_sampai = $_POST['harga_sampai'];
					$jenis_promosi_filter = $_POST['jenis_promosi_filter'];
					$nama_promosi_cari = "";
				}
				?>
				<!-- Berdasarkan Nama -->
				<div class="dis-none panel-search w-full p-t-10 p-b-15">
					...
					<div class="bor8 dis-flex p-l-15">
							<input class="mtext-107 cl2 size-114 plh2 p-r-15" value="<?php echo $nama_promosi_cari;?>" type="text" name="nama_promosi" placeholder="Search">
							<button class="size-113 flex-c-m fs-16 cl2 hov-cl1 trans-04" name="cari_promosi">
					...
					<form method="POST" action="#" style="width: 100%; margin: 0 auto; background-color: #f0f0f0; padding: 20px; border-radius: 8px;">
						...
						<div style="display: flex; flex-wrap: wrap; justify-content: space-between; margin-bottom: 7px;">
							...
							<div style="flex: 1; margin-right: 10px; margin-bottom: 10px;">
								...
								<select name="jenis_promosi_filter" id="jenis-sewa" style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;">
									<option value="" <?= ($jenis_promosi_filter == '') ? 'selected' : '' ?>>All</option>  
									<?php
										$sql2 = "SELECT*FROM d_kategori_promosi order by nama_kategori_promosi";
										$hasil2 = $db->query($sql2);
										while ($baris2 = $hasil2->fetch(PDO::FETCH_ASSOC)) {
											$id_kat_promosi = $baris2['id_kat_promosi'];
											$nama_kategori_promosi = $baris2['nama_kategori_promosi'];
									?>	
										<option value=<?php echo $id_kat_promosi;?> <?= ($id_kat_promosi == $jenis_promosi_filter) ? 'selected' : '' ?>><?php echo $nama_kategori_promosi;?></option>
									<?php
										}
									?>
								</select>
						...
						<!-- Tombol Pencarian -->
						<button name="cari_promosi_filter" type="submit" style="width: 100%; padding: 10px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">
			...
			<div id="cards-container" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
				...
				if ($jenis_promosi_filter == '') {
					$jenis_promosi_filter2 = 'm.id_kat_promosi is not null';
				}
				else {
					$jenis_promosi_filter2 = 'm.id_kat_promosi = '. $jenis_promosi_filter;
				}
				...
				$sql = "SELECT m.nama_promosi, m.keterangan, m.id_promosi, substring(u.fname FROM '\S+$') fname, m.filename,
				m.jml_klik, j.nama_kategori_promosi, f.nama_fakultas, p.nama_prodi, u.kode_fakultas, m.id_kat_promosi, u.kode_univ
				...
				where 
				($jenis_promosi_filter2)
				AND (u.kode_univ = v.kode_univ)
				AND (upper(m.nama_promosi) like upper('%$nama_promosi_cari%'))
				";
                $hasil = $db->query($sql);

                while ($baris = $hasil->fetch(PDO::FETCH_ASSOC)) {					
					$nama_promosi = $baris['nama_promosi'];
                    $id_promosi = $baris['id_promosi'];
					$keterangan = $baris['keterangan'];
                ?>
				<form action="promosi_mahasiswa_view.php?md=<?php echo $id_promosi;?>&fk=<?php echo $jenis_promosi_filter;?>" method="POST" class="card">
					<input type="hidden" name="kosan_id" value="1" />
					<button type="submit" value="<?php echo $id_promosi; ?>" name="klik_<?php echo $id_promosi; ?>">
						<div class="rounded-xl overflow-hidden shadow-md border bg-white hover:shadow-lg transition">
							<img src="<?php echo 'uploads/promosi/'. $baris['filename'];?>" class="w-full h-48 object-cover" alt="" />
							<div class="p-4">
								<p class="text-sm text-red-600 font-semibold mb-1" style="text-align:left">🏷️ <?php echo $baris['nama_kategori_promosi'];?></p>
								<h2 class="text-lg font-semibold text-gray-800" style="text-align:justify"><?php echo $nama_promosi;?></h2>
