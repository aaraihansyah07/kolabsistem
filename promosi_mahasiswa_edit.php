<?php
    ...
    $id_promosi = $_POST['to_detail'];
	
?>
<html lang="en">
<head>
	<title>Edit Promosi Usaha</title>
	<meta charset="UTF-8">
...
<body>
	...
	<section class="sec-relate-product bg0 p-t-45 p-b-105">
		<div class="container">
			<a href='promosi_mahasiswa_upload.php'><button class="btn btn-primary ms-auto"><i class="fa-solid fa-arrow-left"></i> Promosi Saya</button></a>								
            <form action='promosi_mahasiswa_hapus.php?md=<?php echo $id_promosi;?>' style="float:right" method="POST" 
			onsubmit="return confirm('Yakin ingin menghapus usaha ini ? Pengunjung lain tidak akan lagi bisa melihat usaha ini');">
				<button class="btn btn-danger ms-auto"><i class="fa-solid fa-trash"></i> Hapus</button>							
			</form>
			
			<h3 class="ltext-106 cl5 txt-center">
				Edit Promosi Usaha
			</h3>
            <form class="w-full" method="post" action="promosi_mahasiswa_edit_proses.php" enctype="multipart/form-data">
                <div class="flex-w flex-m p-t-50 p-b-23">
                </div>

            	<input id="name" type="text" name="uuiduser" value="<?php echo $uuiduser;?>" hidden/>
                <input id="name" type="text" name="id_promosi" value="<?php echo $id_promosi;?>" hidden/>
                <?php
                    $sql = "SELECT id_kat_promosi, nama_promosi, filename, keterangan, link
				...
				<div class="row p-b-25">
					<div class="col-sm-4 p-b-5">
						...
						<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
							<input placeholder="Cth : Laundry Murah Meriah KolabSistem" value="<?php echo $baris['nama_promosi'];?>" class="size-111 bor8 stext-102 cl2 p-lr-20" id="name" type="text" name="nama_promosi" required>
						</div>
					</div>

                    <div class="col-sm-4 p-b-5">
						<label class="stext-102 cl3" for="review">Kategori Bidang Usaha</label>
						<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
							<select class="form-control" name="id_kat_promosi" required autofocus>
                                ...
                                while ($barisb = $hasilb->fetch(PDO::FETCH_ASSOC)) {     
									$id_kat_promosi = $barisb['id_kat_promosi'];
									$nama_kategori_promosi = $barisb['nama_kategori_promosi'];                                   
                                ?>
      								<option value=<?php echo $id_kat_promosi;?> <?= ($id_kat_promosi == $baris['id_kat_promosi']) ? 'selected' : '' ?>><?php echo $nama_kategori_promosi;?></option>
                                <?php
                                }
                                ?>
							</select>
                    ...
                    <div class="col-sm-4 p-b-5">
						<label class="stext-102 cl3" for="review">Link Medsos/Bidang Usaha</label>
						<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
							<input value="<?php echo $baris['link'];?>" class="size-111 bor8 stext-102 cl2 p-lr-20" id="name" type="text" name="link">
						</div>
					</div>
				</div>
				...
				<div class="row p-b-25" style="margin-top:-2%">
					<div class="col-12 p-b-5">
						...
						<textarea maxlength="300" class="size-110 bor8 stext-102 cl2 p-lr-20 p-tb-10" style="height:150px" placeholder="Deskripsikan informasi penting bidang usaha" id="deskripsi" name="keterangan"><?php echo $baris['keterangan'];?></textarea>
					</div>
				</div>
                    ...
                    <div id="fileContainer" class="flex flex-wrap justify-center gap-4">
						<div class="file-group flex flex-col items-center bg-gray-200 p-4 rounded-lg shadow-md relative" style="width:400px;">
							<label class="cursor-pointer bg-blue-500 text-white px-4 py-2 rounded-md text-sm">
								Upload Gambar Promosi Usaha (Maks. 1 Mb)
								<input type="file" name="filename" accept="image/*" class="file hidden" onchange="previewImage(event, this)">
							</label><br>
							...
							<div class="preview w-full mt-2 bg-white flex items-center justify-center border border-gray-300 rounded-md" style="height:200px;">
								<img src="uploads/promosi/<?php echo $baris['filename']; ?>" style="width: 100%; height: 100%; object-fit: contain; object-position: center;" alt="Preview">
							</div>
						</div>
					</div>
				</div><br>
