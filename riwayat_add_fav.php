	...
	<div class="container">
		<div class="bread-crumb flex-w p-l-25 p-r-15 p-t-30 p-lr-0-lg">
			...
			<a href="#" class="stext-109 cl8 hov-cl1 trans-04">
				Riwayat Penambahan ke Favourite
				<i class="fa fa-angle-right m-l-9 m-r-10" aria-hidden="true"></i>
			</a>
		</div>
	</div>
						...
						<div style="display: flex; flex-wrap: wrap; justify-content: space-between; margin-bottom: 7px;">
							...
							<div style="flex: 1; margin-right: 10px; margin-bottom: 10px;">
								...
								<select name="jenis_notif_filter" id="jenis-sewa" style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box;">
									...
									<option value="UDG" <?= ($jenis_notif_filter == 'UDG') ? 'selected' : '' ?>>Undangan Berkolaborasi dari User Lain</option>  
									<option value="KON" <?= ($jenis_notif_filter == 'KON') ? 'selected' : '' ?>>Konfirmasi Kolaborasi dari Undangan Saya</option>  
								</select>
							</div>
						</div>

						<!-- Tombol Pencarian -->
						<button name="cari_porto_filter" type="submit" style="width: 100%; padding: 10px; font-size: 16px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">
							<i class="fa-solid fa-refresh"></i> Refresh</button>
					</form>
				</div>
			</div>
				...
				<?php
					...
					$sql = "
						SELECT 
							...
							f.nama_fakultas||' '||u.kode_univ kode_fakultas,
							fav.uuiduserpemilik,
							r.nama_porto,
							to_char(fav.createdate, 'DD MONTH YYYY HH24:MI') createdate,
							st_konfirmasi,
							uk.uuidporto
						FROM log_favourite_pencari_porto fav
						LEFT JOIN user_pengguna u ON u.uuiduser = fav.uuiduserfav
						LEFT JOIN d_portofolio r ON r.uuidporto = fav.uuidporto
						LEFT JOIN f_undangan_kerjasama uk ON uk.id_undangan = fav.id_undangan
						...
						WHERE fav.uuiduserpemilik = :uuiduser
						AND (:jenis_notif = '' OR fav.jenis_notif = :jenis_notif)
					...
					$stmt->execute([
						'uuiduser' => $uuiduser,
						'jenis_notif' => $jenis_notif_filter
					]);
					...
					while ($baris = $stmt->fetch(PDO::FETCH_ASSOC)) {
						...
						if ($baris['role_id'] == 2) { // DOSEN
							if ($baris['jenis_notif'] == 'FAV') {
								$caption = "<span class='font-semibold text-red-600'>{$baris['nama_awal']} (Dosen {$baris['kode_univ']})</span>
											<span class='text-gray-600'>menambahkan portofolio</span>
											<span class='font-semibold text-green-600'>{$baris['nama_porto']}</span>
											<span class='text-gray-600'>ke favorit</span>";
							} 
							else if ($baris['jenis_notif'] == 'KON') {
								if ($baris['st_konfirmasi'] == 'Y') {
									$st_konfirmasi = 'menyetujui';
								}
								else {
									$st_konfirmasi = 'menolak';
								}

								$caption = "<span class='font-semibold text-red-600'>{$baris['nama_awal']} dari {$baris['kode_fakultas']}</span>
											<span class='text-gray-600'>".$st_konfirmasi." undangan kolaborasi Anda</span>";
								$caption2 = "\"{$baris['ket_kolab']}\"";
							}
							else {
								$caption = "<span class='font-semibold text-red-600'>{$baris['nama_awal']} (Dosen {$baris['kode_univ']})</span>
											mengundangmu untuk
						...
						elseif ($baris['role_id'] == 3) { // PERUSAHAAN
							if ($baris['jenis_notif'] == 'FAV') {
								$caption = "<span class='font-semibold text-red-600'>{$baris['nama_awal']} dari perusahaan {$baris['nama_perusahaan']}</span>
											<span class='text-gray-600'>menambahkan portofolio</span>
											<span class='font-semibold text-green-600'>{$baris['nama_porto']}</span>
											<span class='text-gray-600'>ke favorit</span>";
							} 
								...
								$caption = "<span class='font-semibold text-red-600'>{$baris['nama_awal']} dari {$baris['nama_perusahaan']}</span>
											mengundangmu untuk
											<span class='font-semibold text-green-600'>{$baris['ket_kerjasama']}</span>";
								$caption2 = "\"{$baris['ket_kolab']}\"";
							}
						...
						elseif ($baris['role_id'] == 1) { // MAHASISWA
							if ($baris['jenis_notif'] == 'FAV') {
								$caption = "<span class='font-semibold text-red-600'>{$baris['nama_awal']} dari {$baris['kode_fakultas']}</span>
											<span class='text-gray-600'>menambahkan portofolio</span>
											<span class='font-semibold text-green-600'>{$baris['nama_porto']}</span>
				...
				<div class="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm" id="cards-container">
					...
					<div class="flex-shrink-0">
						<div class="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
							<?php if ($baris['jenis_notif'] == 'FAV') { ?>
								<i class="zmdi zmdi-favorite text-xl text-red-600"></i>
							<?php } else { ?>
								<i class="zmdi zmdi-email text-xl text-orange-500"></i>
							<?php } ?>
						</div>
					</div>
					...
					<div class="ml-4 flex justify-between w-full">
						...
						<!-- RIGHT BUTTON -->
						<?php if ($baris['jenis_notif'] !== 'FAV' AND $baris['st_konfirmasi'] == NULL) { ?>
						<div class="flex items-start">
							<form method="post" action="konfirmasi_undangan.php">
						...
						<?php if ($baris['jenis_notif'] == 'UDG' AND $baris['st_konfirmasi'] !== NULL) { 
							if ($baris['st_konfirmasi'] == 'N') {
								$caption_konfirmasi = "
								<button name='konfirmasi' type='submit'
								...
								Anda Menyetujui Undangan Ini
								</button>
								";
							}
						?>
						<div class="flex items-start">
							<?php echo $caption_konfirmasi;?>
						</div>
						<?php } ?>
						
						<?php if ($baris['jenis_notif'] == 'KON' AND $baris['st_konfirmasi'] !== NULL) { ?>
						<div class="flex items-start">
