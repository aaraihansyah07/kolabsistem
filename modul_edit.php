...
<body>
	...
	<section class="sec-relate-product bg0 p-t-45 p-b-105">
		<div class="container">
			<a href='modul_saya.php'><button class="btn btn-primary ms-auto">
				<i class="fa-solid fa-arrow-left"></i> Portofolio Saya
			</button></a>
            <form action='modul_hapus.php?md=<?php echo $uuidporto;?>' style="float:right" method="POST" 
			onsubmit="return confirm('Yakin ingin menghapus portofolio ini ? Mahasiswa/dosen/pencari talent tidak akan lagi bisa melihat portofolio ini');">
            ...
            <form class="w-full" method="post" action="modul_edit_proses.php" enctype="multipart/form-data">
                ...
                <?php
                    $sql = "SELECT id_kat_porto, nama_porto, filename, keterangan, thumbnail_filename
				...
				<div class="row p-b-25">
					<div class="col-sm-6 p-b-5">
						<label class="stext-102 cl3" for="review">Nama Protofolio</label>
						<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
							<input placeholder="Cth : Animasi 3d" value="<?php echo $baris['nama_porto'];?>" class="size-111 bor8 stext-102 cl2 p-lr-20" id="name" type="text" name="nama_porto" required>
						</div>
					</div>

                    <div class="col-sm-6 p-b-5">
						<label class="stext-102 cl3" for="review">Kategori Portofolio</label>
						<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
							<select class="form-control" name="id_kat_porto" required autofocus>
								<option value="">--Pilih--</option>
                                <?php
                                $sqlb = "SELECT id_kat_porto, nama_kategori_porto FROM d_kategori_portofolio";
                                $hasilb = $db->query($sqlb);
                                while ($barisb = $hasilb->fetch(PDO::FETCH_ASSOC)) {     
									$id_kat_porto = $barisb['id_kat_porto'];
									$nama_kategori_porto = $barisb['nama_kategori_porto'];                                   
                                ?>
      								<option value=<?php echo $id_kat_porto;?> <?= ($id_kat_porto == $baris['id_kat_porto']) ? 'selected' : '' ?>><?php echo $nama_kategori_porto;?></option>
                                <?php
                                }
                                ?>
							</select>
				...
				<div class="row p-b-25" style="margin-top:-2%">
					<div class="col-12 p-b-5">
						...
						<textarea maxlength="300" class="size-110 bor8 stext-102 cl2 p-lr-20 p-tb-10" style="height:150px" placeholder="Deskripsikan secara singkat portofolio" id="deskripsi" name="keterangan"><?php echo $baris['keterangan'];?></textarea>
					</div>
				</div>
                    ...
                    <div id="fileContainer" class="flex flex-wrap justify-center gap-4">
						<div class="file-group flex flex-col items-center bg-gray-200 p-4 rounded-lg shadow-md relative" style="width:400px;">
							<label class="cursor-pointer bg-blue-500 text-white px-4 py-2 rounded-md text-sm">
								Upload Gambar Preview 1 (Maks. 1 Mb)
								<input type="file" name="thumbnail_filename" accept="image/*" class="file hidden" onchange="previewImage(event, this)">
							</label><br>
							...
							<div class="preview w-full mt-2 bg-white flex items-center justify-center border border-gray-300 rounded-md" style="height:200px;">
								<img src="uploads/thumbnail/<?php echo $baris['thumbnail_filename']; ?>" style="width: 100%; height: 100%; object-fit: contain; object-position: center;" alt="Preview">
							</div>
						</div>
					</div>
				</div><br>
