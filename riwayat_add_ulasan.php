...
<head>
    ...
    .notif-wishlist-count {position: absolute;top: -5px;right: -5px;background: red;color: white;font-size: 12px;font-weight: bold;padding: 3px 7px;border-radius: 50%;}	</style>
	<meta charset="UTF-8">
	...
	<section class="bg0 p-t-23 p-b-140">
		<div class="container">
            ...
            <div class="space-y-4">
                <!-- Notifikasi 1 -->
                 <?php
                    $sql4 = "UPDATE ulasan_user set st_view = 'Y' where st_view is null and uuiduserpemilik = :uuiduserpemilik";
                    $stmt4 = $db->prepare($sql4);
                    $stmt4->execute(['uuiduserpemilik' => $uuiduser]);

                    $sql = "SELECT r.nama_modul, uuidulasan, uuiduserulas, rating, ulasan, to_char(rev.createdate, 'DD MONTH YYYY HH24:MI') createdate, 
					st_view, uuiduserpemilik, split_part(fname, ' ', 1) nama_awal
					FROM ulasan_user rev
					left join user_pengguna u on rev.uuiduserulas = u.uuiduser
					left join d_modul r on r.uuidmodul = rev.uuidmodul
                    where rev.uuiduserpemilik = '$uuiduser' order by createdate desc";
                    $hasil = $db->query($sql);
                ...
                <div class="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm">
                    ...
                    <div class="ml-4">
                        <p class="text-gray-700 text-sm sm:text-base">
                            <span class="font-semibold text-red-600"><?php echo $baris['nama_awal'];?></span> memberikan <?php echo $rating;?> pada ruangan perangkat ajar <span class="font-semibold text-green-700"><?php echo $baris['nama_modul'];?></span>.
                        </p>
						<p class="text-xs text-black-400 mt-1"><?php echo '"'. $baris['ulasan']. '"';?></p>
