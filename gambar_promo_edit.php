...
<html lang="en">
<head>
	<title>Upload Gambar Promosi Usaha</title>
	<meta charset="UTF-8">
	...
	<section class="sec-relate-product bg0 p-t-45 p-b-105">
		<div class="container">
            ...
            <h3 class="ltext-106 cl5 txt-center">
				Unggah Gambar Promosi Usaha
			</h3>
            <form class="w-full" method="post" action="gambar_promo_edit_proses.php" enctype="multipart/form-data">
				...
				<div class="max-w-9xl mx-auto bg-white p-6 rounded-lg shadow-md">
					<div id="fileContainer" class="flex flex-wrap justify-center gap-4">
						<?php
                            $sql2b = "SELECT*FROM d_gambar_promo where uuiduser = '$uuiduser'";
							$hasil2b = $db->query($sql2b);
                            $baris2b = $hasil2b->fetch(PDO::FETCH_ASSOC);

							$sql2 = "SELECT*FROM d_gambar_promo where uuiduser = '$uuiduser'";
						...
						<div class="file-group flex flex-col items-center bg-gray-200 p-4 rounded-lg shadow-md relative" style="width:400px;">
							...
							<input type="hidden" name="uuidgambarpromo[]" value="<?php echo $baris2['uuidgambarpromo'];?>"/>
							<div class="preview w-full mt-2 bg-white flex items-center justify-center border border-gray-300 rounded-md" style="height:200px">
								<span class="text-gray-500 text-xs"><img src='uploads/promosi_usaha/<?php echo $baris2['filename'];?>' style="width: 100%; height: 100%; object-fit: contain; object-position: center;;"/></span>
							</div>
							<button type="button" class="remove-btn absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center shadow-md" onclick="removeInput(this)">✖</button>
							<button value="<?php echo $baris2['uuidgambarpromo'];?>" type="submit" class="remove-btn absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center shadow-md" name="hapus_gambar">✖</button>
						</div>
						<?php
							}
						?>
					...
					<!-- Tombol Tambah Input File -->
                     <?php
                        if (!isset($baris2b['uuidgambarpromo'])) {
                            echo "<div class='text-center mt-4'>
                                <button id='btnAdd' type='button' onclick='addFileInput()' class='bg-blue-500 text-white px-4 py-2 rounded-md'>Tambah Gambar</button>
                            </div>";
                        }
                        else {
                            echo "";
...
function removeInput(button) {
    ...
    // Tampilkan kembali tombol tambah gambar
    const btnAdd = document.getElementById("btnAdd");
    btnAdd.style.display = "inline-block"; // atau "flex" sesuai desain
}


function addFileInput() {
    // Sembunyikan tombol tambah gambar setelah ditekan
    const btnAdd = document.getElementById("btnAdd");
